...
import { GridFilter, GridFilterOperator, GridSearch, GridSort } from './data-grid-provider';
...
export class DataGridUtils {
  ...
  static formatDate(dateString: string, type: 'EN' | 'it' = 'EN'): string {
    ...
    const isMatchISO = dateString.match(regExpISO);
    const isMatchIT = dateString.match(regExpIT);
    if (!isMatchISO && !isMatchIT) return '';

    if (isMatchIT) {
      if (dateString.includes('-')) dateString = `${dateString.split('-')[2]}-${dateString.split('-')[1]}-${dateString.split('-')[0]}`;
      else if (dateString.includes('/')) dateString = `${dateString.split('/')[2]}-${dateString.split('/')[1]}-${dateString.split('/')[0]}`;
    }
    ...
    if (type === 'EN') return `${year}-${month}-${day}`;
    return `${day}-${month}-${year} `;
  }

  static matchesLocalSearch(value: any, searchText: string, exact = false, searchType: any = ''): boolean {
    if (value === null || typeof value === 'undefined' || Array.isArray(value)) return false;
    ...
    if (searchType === 'startsWith') return itemValue.startsWith(valueToSearch);
    return itemValue.includes(valueToSearch);
  }

  static filterNonRemoteDataSource(array: any[], dataField: string | number, searchText: string, exact = false, searchType: any = ''): any[] {
    return array.filter(item => DataGridUtils.matchesLocalSearch(item?.[dataField], searchText, exact, searchType));
  }

  static calculateColumnSummary(rows: any[], cols: ColData): number {
    if (!cols.dataField || !cols.showInSummary) return 0;
  ...
  static getOriginalColumn(columns: Colonne[] | undefined, field: string): any {
    for (const group of columns ?? []) {
      if ((group as any)?.dataField === field) return group;
  ...
  static getProviderSearchColumns(colsHeader: ColData[], columns: Colonne[] | undefined): GridFilterColumnMetadata[] {
    return colsHeader
  ...
  static getProviderFilterColumn(colsHeader: ColData[], columns: Colonne[] | undefined, field: string): GridFilterColumnMetadata | undefined {
  ...
  static resolveProviderFilterInputValue(colsHeader: ColData[], columns: Colonne[] | undefined, field: string, value: string): unknown {
    if (value === '') return '';
    const column = colsHeader.find(currentColumn => currentColumn.dataField === field);
  ...
  static cloneSearch(search?: GridSearch): GridSearch | undefined {
    ...
    return { value: search.value, conditions: search.conditions.map(condition => ({ ...condition })) };
  }
  ...
  static createMockItem<T>(item?: T): T | undefined {
    ...
    Object.keys(mock).forEach(key => { mock[key] = null; });
    return mock as T;
  }
}
