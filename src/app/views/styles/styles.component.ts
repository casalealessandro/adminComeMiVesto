...
@Component({
  standalone: true, selector: 'app-styles', imports: [CommonModule, DynamicFormComponent, DataGridComponent],
  templateUrl: './styles.component.html', styleUrl: './styles.component.scss',
})
export class StylesComponent {
  ...
  load(): void {
    ...
    this.api.getStyles().pipe(finalize(() => this.loading = false)).subscribe({
      next: styles => { this.styles = [...styles].sort((a, b) => a.order - b.order); this.updateGrid(); },
  ...
  handleForm(event: { name: string; formData: Record<string, unknown> }): void {
    ...
    this.editing = {
      ...
      value: String(event.formData['value'] ?? '').trim(), order: Number(event.formData['order']), gender, parent: null,
    };
    this.save();
  }
  ...
  async selectImage(event: Event, gender: StyleGender): Promise<void> {
    ...
    this.processingImage = gender; this.error = '';
    try {
      const image = await this.prepareImage(file, gender);
      if (image.imageBase64.length > MAX_IMAGE_BASE64_LENGTH) { this.error = "L'immagine è ancora troppo grande. Seleziona un'immagine più leggera."; return; }
      this.editing.images = { ...this.editing.images, [gender]: image }; this.imageChanged[gender] = true;
    } catch { this.error = 'Impossibile elaborare l’immagine selezionata.'; }
    finally { this.processingImage = null; input.value = ''; }
  }
  removeImage(gender: StyleGender): void { this.editing.images = { ...this.editing.images, [gender]: null }; this.imageChanged[gender] = true; }
  ...
  remove(style: OutfitStyle): void {
    confirm(`Eliminare lo stile ${style.value}?`, 'Conferma', yes => yes && this.api.deleteStyle(style.id).subscribe({
      next: () => { this.styles = this.styles.filter(item => item.id !== style.id); this.updateGrid(); },
      error: error => { this.error = error.status === 409 ? 'Lo stile è utilizzato e non può essere eliminato.' : 'Eliminazione non riuscita.'; },
    }));
  }

  private save(): void {
    if (!isStyleValid(this.editing)) { this.error = 'Compila codice e nome e seleziona almeno un gender.'; return; }
    if (this.genders.some(gender => (this.editing.images?.[gender]?.imageBase64.length ?? 0) > MAX_IMAGE_BASE64_LENGTH)) { this.error = "L'immagine è ancora troppo grande. Seleziona un'immagine più leggera."; return; }
    this.loading = true; this.error = '';
  ...
  private updateGrid(): void { this.gridRows = this.styles.map(style => ({ ...style, imageU: this.imageUrl(style.images?.U) ?? '', imageD: this.imageUrl(style.images?.D) ?? '', genderLabel: style.gender.map(item => item === 'U' ? 'Uomo' : 'Donna').join(', ') })); }
  private emptyStyle(): OutfitStyle { return { id: '', value: '', parent: null, order: 0, gender: [] }; }
  private createPayload(): OutfitStyle {
    const images: OutfitStyleImages = {}; if (this.editing.images?.U) images.U = this.editing.images.U; if (this.editing.images?.D) images.D = this.editing.images.D;
  ...
  private prepareImage(file: File, gender: StyleGender): Promise<OutfitStyleImage> {
    return new Promise((resolve, reject) => { const reader = new FileReader(); reader.onerror = () => reject(reader.error); reader.onload = () => {
      const source = new Image(); source.onerror = reject; source.onload = () => { const scale = Math.min(600 / source.width, 750 / source.height, 1); const canvas = document.createElement('canvas'); canvas.width = Math.max(1, Math.round(source.width * scale)); canvas.height = Math.max(1, Math.round(source.height * scale)); const context = canvas.getContext('2d'); if (!context) { reject(new Error('Canvas non disponibile')); return; } context.drawImage(source, 0, 0, canvas.width, canvas.height); resolve({ imageBase64: canvas.toDataURL('image/jpeg', .8).split(',')[1], imageMimeType: 'image/jpeg', imageFileName: `${this.editing.id.trim() || 'style'}-${gender}.jpg` }); }; source.src = String(reader.result);
